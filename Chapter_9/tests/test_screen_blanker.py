...
#
#   python3 Chapter_9/tests/test_screen_blanker.py
...
class TestScreenBlanker(unittest.TestCase):
    ...
    def test_blanks_after_timeout_with_no_activity(self):
        backlight = FakeBacklight()
        blanker = ScreenBlanker(backlight, timeout=10.0)
        blanker.update(False, 0.0)
    ...
    def test_activity_resets_the_idle_clock(self):
        ...
        blanker = ScreenBlanker(backlight, timeout=10.0)
        blanker.update(False, 0.0)
        blanker.update(True, 9.0)  # activity just before timeout
        blanker.update(False, 18.0)  # 9s since -- still under 10s
        self.assertTrue(blanker.lit)

    def test_activity_wakes_a_blanked_screen(self):
        backlight = FakeBacklight()
        blanker = ScreenBlanker(backlight, timeout=10.0)
        blanker.update(False, 0.0)
        blanker.update(False, 10.0)
